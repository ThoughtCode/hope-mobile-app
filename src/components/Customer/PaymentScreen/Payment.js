...
import {Text,TextInput,View,Image,TouchableOpacity,FlatList,Dimensions,Alert,Linking,ScrollView,KeyboardAvoidingView} from 'react-native';
...
export default class Payment extends React.Component {
  ...
  calculateDiscount = (serviceId, services, discountPercentage, total) => {
    let service = 0;
    ...
    if (service.length != []) {
      ...
      calculate = s.filter(function(service){
        ...
        calculateReady = total * discountPercentage / 100;
      });
  ...
  render() {
    ...
    if(this.state.codeAplay == true){
      discount = this.calculateDiscount(this.state.dataCodeResponse.promotion.data.attributes.service.id, this.state.serviceType, this.state.dataCodeResponse.promotion.data.attributes.discount, total)
      if(discount != false){
        total = (total - discount)
      }else{
        discount = 0
        Alert.alert(globals.APP_NAME,"CÓDIGO Promocional no valido para este Servicio",[
          {text: 'OK', onPress: () => this.setState({textInputStatus: true,codeAplay: false,dataCodeResponse:[]})},
        ])
      }
    }else{
      total = total;
              ...
              <View style={{ flex: 0.9 }}>
                ...
                {(this.state.codeAplay == true) ? (
                  <View style={styles.childContainer}>
                    <View style={styles.itemView}>
                      <Text style={{ flex: 0.8, fontSize: 16,fontFamily: "helvetica" }}>
                        Descuento Promoción del {this.state.dataCodeResponse.promotion.data.attributes.discount}%
                      </Text>
                      <Text style={{ flex: 0.2, fontSize: 16,fontFamily: "helvetica" }}>
                        $-{discount.toFixed(2)}
                      </Text>
