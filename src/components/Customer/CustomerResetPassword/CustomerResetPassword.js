...
const styles = require('./CustomerResetPasswordStyles');

export default class CustomerResetPassword extends Component {
  constructor(props) {
    super(props);
    this.state = {
      email: '',
      ...
      if (this.state.resetPassword === true) {
        fetch(updatePasswordURL, {
          ...
          if (response.status === 404) {
            ...
            response.json().then(() => {
              this.props.navigation.navigate('CustomerLogin', {recover_password_text: "Contraseña recuperada. \n Utilice nuevas credenciales \n para ingresar"});
            });
          }
